...
void setup() {
  ...
  prog = EEPROM.read(addr);
  intensity = EEPROM.read(addr);
  ...
  EEPROM.write(addr, prog);
  EEPROM.write(addr2, intensity);
...
void loop() {
  if(prog==0)
  {
  colorWipe(strip.Color(0, 0, 50), 10);  
  colorWipe(strip.Color(0, 50, 50), 10);
